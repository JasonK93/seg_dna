...
def cut():
    ...
    for i in range(500):
        try:
            import numpy as np
            "load image data"
            Img_Original = io.imread('data/step2/pic7/{}.png'.format(i))  # Gray image, rgb images need pre-conversion
...
####################
            ...
            for (r, c) in skel_coords:
                matrix[r, c] = 255
                head_num += 1
            cv2.imwrite("data/head/a{}.png".format(i), Img_Original)
            cv2.imwrite("data/head/{}.png".format(i), matrix)
...
def test():
    import numpy as np
    "load image data"
    Img_Original = io.imread('data/step2/pic7/91.png') # Gray image, rgb images need pre-conversion
    kernel_2 = np.ones((2, 2), np.uint8)
    tmp = np.zeros((Img_Original.shape[0] + 40, Img_Original.shape[1] + 40))
    tmp[20:-20, 20:-20] = Img_Original
    Img_Original = tmp
